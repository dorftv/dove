...
class sceneMixer(Mixer):
    ...
    def build(self):
        ...
        self.add_pipeline(f"videotestsrc is-live=true pattern=2 ! { self.get_caps('video') } ! "
            f" compositor latency=70 zero-size-is-unscaled=false background=black force-live=true ignore-inactive-pads=true name=videomixer_{self.data.uid} sink_0::alpha=1 ! videorate ! videoconvert ! videoscale ! { self.get_caps('video') } ! "
            f" {self.get_video_end()} "
            f" audiotestsrc wave=4 ! { self.get_caps('audio') } ! liveadder  latency=70  name=audiomixer_{self.data.uid} force-live=true ignore-inactive-pads=true !  audioconvert ! audiorate ! audioresample ! { self.get_caps('audio') } ! "
            + self.get_audio_end())

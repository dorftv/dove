...
class SrtserversinkOutput(Output):
    ...
    def build(self):
        ...
        if input.has_audio_or_video("audio"):
                ...
                pipeline_audio_str = f" {self.get_audio_start()}  audioconvert ! audioresample ! {audio_caps} ! { aenc_str } ! queue leaky=downstream ! mux."
        ...
        f" x264enc {self.data.x264_opts}  ! video/x-h264,profile={ self.data.h264_profile } ! h264parse ! queue leaky=downstream !  "
